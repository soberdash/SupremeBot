module.exports = async (client, message, args) => {
    ...
    if(!args[0]){
        ...
        let pr = args[1]
        if(!perms) return message.channel.send('<a:ani_no:709633289106882590> No tienes permisos para realizar esta acción. Permiso requerido: ``MANAGE_GUILD``')
        ...
        const nw = await prefixSchema.findOneAndUpdate({ guildID: message.guild.id }, { $set: { prefix: '>' } }, { new: true }).then(() => {
            message.channel.send(`<a:ani_yes:709633371122434050> Listo! He reiniciado el prefijo para este servidor!`)
        }).catch((err) => {
            message.channel.send('<a:ani_no:709633289106882590> Ha ocurrido un error! ('+err+')')
        })
    }else return message.channel.send('<:info:712004394727505981> Puedes utilizar: ``set, reset``')
}
